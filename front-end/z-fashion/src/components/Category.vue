      ...
      <template #overlay>
        <a-menu>
          <a-sub-menu v-for="(category, index) in categories" :key="index">
            <template #title>
              <router-link style="text-decoration: none;" :to="`/clothing/${category.name}`">{{ category.name
                }}</router-link>
            </template>
            <a-menu-item v-for="(child, index) in category.children" :key="index">
              <router-link style="text-decoration: none;" :to="`/clothing/${category.name}/${child.id}`">{{
                child.name
              }}</router-link>
            </a-menu-item>
          </a-sub-menu>
