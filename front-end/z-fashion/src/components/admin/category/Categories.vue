<template>
  <div
    class="col-10 position-absolute"
    style="top: 10%; left: 16%; width: 1121px"
  >
    <div class="card">
      <div class="card">
        ...
        <div class="card-body">
          <div class="d-flex justify-content-between col-6">
            <a-button type="primary" style="height: 38px"
          ...
          <div class="d-flex justify-content-between col-6 float-end">
            <div class="result-search" v-show="searched">
              <span style="color: #ccc" class="d-inline-block"
                >Kết quả tìm kiếm: {{ categories.length }}</span
              >
            </div>
            <div class="filter">
              <a-dropdown>
                <a-button style="width: 50px"
                  ><FilterOutlined
                    class="filter-icon ant-dropdown-link"
                  ></FilterOutlined
                ></a-button>
                <template #overlay>
                  <a-menu @click="handleMenuClick">
                    <a-menu-item
                      :key="index"
                      @click="filterCateChildByCateParent(parent.id)"
            ...
            <tbody>
              <tr v-for="category in paginatedCategories" :key="category.id">
                ...
                <td>
                  ...
                  >
                    <EditTwoTone
                      two-tone-color="#FA7921"
                      style="font-size: 30px; cursor: pointer"
                    />
                  </router-link>
                  <DeleteTwoTone
                    two-tone-color="red"
                    style="font-size: 30px; cursor: pointer; margin-left: 6px"
...
const resultsBySearch = () => {
  const token = localStorage.getItem("token");
  auth.defaults.headers.common["Authorization"] = `Bearer ${token}`;
  auth.get(`search/category/${keywords.value}`).then((res) => {
...
const filterCateChildByCateParent = (parent_id) => {
  axios
    .get(`category/child_category/${parent_id}`)
    .then((res) => {
      categories.value = res.data;
    })
    .catch((err) => {
