    ...
    <div class="card">
      <div class="card">
          ...
          <table class="table table-success table-striped mt-2">
            <thead>
              <tr>
                ...
                <th>Giá bán</th>
                <th style="width: 100px">Số lượng tồn kho</th>
                <th>Tình trạng</th>
            ...
            <tbody>
              <tr v-for="product in paginatedProducts" :key="product.id">
                <td>{{ product.id }}</td>
                <td v-if="product.name">
                  {{ product.name.substring(0, 15) + "..." }}
                </td>
                <td v-if="product.category.name || product.parent.name">
                  {{ product.category.name || product.parent.name }}
                </td>
                <td>
                  {{
                ...
                <td v-if="product.inventory > 0">{{ product.inventory }}</td>
                <td v-else>0</td>
                <td v-if="product.status == 0">
                  <div class="available">Còn hàng</div>
                </td>
                <td v-else>
                  <div class="unavailable">Hết hàng</div>
                </td>
                <td v-for="image in product.images" :key="image.index">
                  <img :title="product.name" :alt="product.name" style="width: 60%" v-if="image['url'] !== undefined"
                    :src="image['url']" />
                </td>
        ...
        <div class="card-footer text-center">
          <a-pagination v-model:current="currentPage" :total="products.length" :pageSize="pageSize"
...
const getCateByParentIdIsNull = () => {
  auth
    .get("category/parent_id-is-null")
    .then((res) => {
